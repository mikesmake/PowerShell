...
$smtpFrom = "" 
$smtpTo = @('') 
$messageSubject = "SQL server Job Failures"

#Export Details
$CSVLocation = "C:\Temp\JobFailures.csv"
#############################################################
...
foreach ($instance in $instances){
    $results += (Get-SqlAgentJob -ServerInstance $instance.name).where{$_.LastRunOutcome -eq 'Failed'}
}
...
if($OnScreen -eq "yes"){$results | select name,lastrunduration,LastRunOutcome | ft}
...
$body += $results |  select name,lastrunduration,LastRunOutcome | convertto-html -Head $style 
#Sends mail
